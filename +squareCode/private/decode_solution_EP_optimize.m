function [decodedSolutions, maxClusterSize] = decode_solution_EP_optimize(solution,varargin)
% DECODE_SOLUTION_EP_OPTIMISE decodes the solution from the machine by optimising EP state.
%
% Energy penalty decoding (EP decoding) implies that we only decode when all physical qubits
% corresponding to the logical qubit end up in same state. That is, +4 -> upSpin, -4 ->
% downSpin and 0 otherwise. We term these undecided qubits as ties. On this resulting
...
decodedSolutions = decode_solution_EP(solution); %First decode the solution by EP strategy
