...
%Input :
%  solution         - a num_physical_qubitsxnum_read array from DW2
%Output:
%  decodedSolutions - a num_logical_qubitsxnum_read array which corresponds to
...
numPhyQubits     = size(solution,1); %Finds out the number of physical qubits.
numLogicalQubits = numPhyQubits/4; %Total logical qubits, working or otherwise
%Invalid/non-used qubits default to three, just like on D-Wave.
decodedSolutions = 3*ones(numLogicalQubits,numOfReads);

for logicalQubit=setdiff(0:1:(numLogicalQubits-1),holes)
