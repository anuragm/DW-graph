...
for ii=1:length(keySet)
    ...
    %physical qubits.
    unitCellY  = floor(keySet(ii)/(2*cellSize));
    unitCellX  = mod(keySet(ii),cellSize);
    isDownHalf = (mod(keySet(ii),2*cellSize)>(cellSize-1));

    physicalQubit1 = 8*(cellSize*unitCellY+unitCellX) + (isDownHalf*2); %Top-left qubit.
...
for ii=1:length(keySet)
    ...
    whichLayer  = 2*(mod(currentQubit,2*cellSize)<cellSize)-1; %Backlayer +1, Front layer -1

    perspectiveNgbrs = [(currentQubit+1)*isRightEdge (currentQubit-1)*isLeftEdge ...
                        currentQubit+2*cellSize currentQubit-2*cellSize ...
                        currentQubit+whichLayer*cellSize];
    ...
    for candidate=ngbrCandidates
       ...
       elseif (currentQubit-candidate)== cellSize %Front-back neighbor
       ...
       elseif (currentQubit-candidate) == -cellSize %back-Front neighbor
